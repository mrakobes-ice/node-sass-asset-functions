...
Processor.prototype.opposite_position = function(position){
    // Accepts and returns a sass.types.List (or sass.types.String or sass.types.Number)
    var result = sass.types.List(position.getLength());
    ...
    for(var i = 0; i < position.getLength(); i++){
        ...
        if(pos.constructor === sass.types.String){
            switch(pos.getValue()){
                case "top": result.setValue(i, new sass.types.String("bottom")); break;
                case "bottom": result.setValue(i, new sass.types.String("top")); break;
                case "left": result.setValue(i, new sass.types.String("right")); break;
                case "right": result.setValue(i, new sass.types.String("left")); break;
                default:
                    console.warn(`Cannot determine the opposite position of: ${pos}`);
                    return pos;
            }
        }
        else if(pos.constructor === sass.types.Number){
            if(pos.getUnit() !== '%')
                console.warn(`Cannot determine the opposite position of: ${pos}`);
...
Processor.prototype.inline_font_files = function(files, done){ //files[] = [filepath, format]
    var processed_files = [], count = 0;
...
Processor.prototype.append_selector = function(selector, to_append,done){

};
Processor.prototype.enumerate = function(prefix, _from, through, separator){

};
Processor.prototype.headings = function(_from, _to,done){
...
/* =============================================================== Mathematics ======================================================== */
Processor.prototype.pi = function(){
    return Math.PI;
};
Processor.prototype.sin = function(number){
    return Math.sin(number);
};
Processor.prototype.cos = function(number){
    return Math.cos(number);
};
Processor.prototype.tan = function(number){
    return Math.tan(number);
};
Processor.prototype.asin = function(number){
    return Math.asin(number);
};
Processor.prototype.acos = function(number){
    return Math.acos(number);
};
Processor.prototype.atan = function(number){
    return Math.atan(number);
};
Processor.prototype.e = function(){
    return Math.E;
};
Processor.prototype.logarithm = function(number, base){
    return Math.log(number)/(base ? Math.log(base) : 1);
};
Processor.prototype.sqrt = function(number){
    return Math.sqrt(number);
};
Processor.prototype.pow = function(number, exponent){
    return Math.pow(number, exponent);
};


/* ================================================================ CROSS-BROWSER ================================================================ */
Processor.prototype.prefixed = function(prefix){ //args
    var args = Array.ftom(arguments).splice(1);
    return args.every(x => typeof x === "string" && x.match(prefix + "-"));
};
Processor.prototype.prefix = function(prefix){   //args
    var args = Array.ftom(arguments).splice(1);
    return args.map(x => typeof x === "string" && x.match(prefix + "-") === null ? prefix + '-' + x : x );
};
Processor.prototype["-webkit"] = function(){     //args
    var args = Array.ftom(arguments).splice(1);
    return this.prefix("-webkit", ...args);
};
Processor.prototype["-moz"] = function(){        //args
    var args = Array.ftom(arguments).splice(1);
    return this.prefix("-moz", ...args);
};
Processor.prototype["-o"] = function(){   //args
    var args = Array.ftom(arguments).splice(1);
    return this.prefix("-o", ...args);
};
Processor.prototype["-ms"] = function(){  //args
    var args = Array.ftom(arguments).splice(1);
    return this.prefix("-ms", ...args);
};
Processor.prototype["-svg"] = function(){  //args
    var args = Array.ftom(arguments).splice(1);
    return this.prefix("-svg", ...args);
};
Processor.prototype["-pie"] = function(){  //args
    var args = Array.ftom(arguments).splice(1);
    return this.prefix("-pie", ...args);
};
Processor.prototype["-css2"] = function(){  //args
    var args = Array.ftom(arguments).splice(1);
    return this.prefix("-css2", ...args);
};
